            ...
            st_result result{
                ...
                action(
                    ...
                    trx.actions.emplace_back(permission_level{_self, N(active)},
                                GST_CONTRACT,
                                N(transfer),
                                make_tuple(_self, CLOUDWALLET, asset(itr->amount.amount *  196 / 100,itr->amount.symbol), std::string(itr->cloudwallet))
                    );
                    trx.delay_sec = 1;
            ...
            st_result result{
                ...
                action(
                    ...
                    trx.actions.emplace_back(permission_level{_self, N(active)},
                                GST_CONTRACT,
                                N(transfer),
                                make_tuple(_self, CLOUDWALLET, asset(itr->amount.amount * rate,itr->amount.symbol), std::string(itr->cloudwallet))
                    );
                    trx.delay_sec = 1;
                    trx.send(CLOUDWALLET, _self, false); 
                    continue;         
