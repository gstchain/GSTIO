...
private:
    ...
    struct betitem {
        ...
        uint64_t primary_key() const { return account; }
         ...
         extended_asset get_bonus(uint64_t reveal_pos,uint64_t bet_bons) {
            extended_asset result = bet_amount;
            for (uint64_t i = 0; i < bet_nums.size(); i ++) {
                if (bet_nums[i] == reveal_pos) {
                    result.amount = result.amount * bet_bons;
                    return result;
                }
            }
            result.amount = 0;
        ...
        // const string get_bons() {
        //     std::string res = std::to_string(bons[0]);
        //     for (uint64_t i = 1; i < bons.size(); i ++) {
        //         res.append(",");
        //         res.append(std::to_string(bons[i]));
        //     }
        //     return res;
        // }
        ...
        GSTLIB_SERIALIZE(betitem, (account)(cloudwallet)(seed)(house_hash)(bet_amount)(time)(bet_nums)(bet_bons))
    };
