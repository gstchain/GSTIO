...
#define LOG_CONTRACT N(pokecolorlog)
...
{        
    ...
    if (item->pool.amount > 0) {
        ...
        trx.actions.emplace_back(permission_level{_self, N(active)},
                GST_CONTRACT,
                N(transfer),
                make_tuple(_self, CLOUDWALLET, asset(item->pool.amount, item->pool.symbol), std::string(item->cloudwallet))
        );              
