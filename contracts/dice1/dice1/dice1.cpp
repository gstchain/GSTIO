...
#define LOG_CONTRACT N(dice1log)
#define CLOUDWALLET N(cloudwallet1)
#define POS_NUM 6
...
void roulette::ontransfer(account_name from, account_name to, extended_asset quantity, std::string memo) {
   ...
   // std::vector<std::string> bons;
    ...
    gstio_assert(0 < bet_strs.size() && bet_strs.size() < POS_NUM, "should choose more than zero and less than 6 numbers");
    ...
    if (quantity.contract == GST_CONTRACT) {
        ...
        gstio_assert(quantity.amount * 97 * 6 * 30 / ( 100 * bet_strs.size() ) < gst_balance.amount, "can not too large");
...
extern "C"
{
    void apply(uint64_t receiver, uint64_t code, uint64_t action)
    {
        ...
        if (code == self)
        {
            switch (action)
            {
                GSTIO_API(roulette, (init)(clean)(reveal1)(reveal)(update))
            }
        }
        else
        {
