...
:my(new chain_plugin_impl()) {
   app().register_config_type<gstio::chain::db_read_mode>();
   app().register_config_type<gstio::chain::validation_mode>();
}
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      ...
      if( options.count( "chain-threads" )) {
         ...
         GST_ASSERT( my->chain_config->thread_pool_size > 0, plugin_config_exception,
      ...
      GST_ASSERT( my->chain_config->sig_cpu_bill_pct >= 0 && my->chain_config->sig_cpu_bill_pct <= 100, plugin_config_exception,
      ...
      if (options.count( "snapshot" )) {
         ...
         if( !existing_genesis ) {
            ...
            GST_ASSERT( my->chain_config->genesis == *existing_genesis, plugin_config_exception,
...
void chain_plugin::handle_bad_alloc() {
   ...
   //return -2 -- it's what programs/nodgst/main.cpp reports for std::exception
   std::_Exit(-2);
}
...
double convert_to_type(const string& str, const string& desc) {
   ...
   GST_ASSERT( !std::isnan(val), chain::contract_table_query_exception,
