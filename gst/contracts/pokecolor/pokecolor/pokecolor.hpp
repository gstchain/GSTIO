    ...
    struct betitem {
        ...
        uint64_t primary_key() const { return account; }
         ...
         extended_asset get_bonus(uint64_t reveal_pos,uint64_t bet_num) {
            ...
            int  res_color ;//开奖 花色
            if((reveal_pos % 13) > 0) {
                res_color = reveal_pos / 13 + 1;
             }else{
                 res_color = reveal_pos / 13 ;
             }         
            if(res_color ==1 || res_color == 3)
            {
                res_color = 1;//black
            }
            else
            ...
            if(bet_num == 0){     // 第一回合   
                if(bet_nums[0]==res_color)
                    {
                        res.amount = bet_amount.amount * 2 * 97 / 100 ;
                        this->result = "win";
                        switch (res_color)
                        {
                        case 1:
                            this->b = res.amount * 51 * 97 / (100 * 25); 
                            this->r = res.amount * 51 * 97 / (100 * 26);                        
                            break;
                        case 2:
                            this->b = res.amount * 51 * 97 / (100 * 26);
                            this->r = res.amount * 51 * 97 / (100 * 25);
                            break;
                        default:
                            break;
                        }
            ...
            else  // 二次投注
            {
                if(bet_num == res_color)
                {
                    ...
                    res.amount = this->pool.amount * (52 - this->round_result.size()) * 97 / (100 * (26 - i ));               
                    this->result = "win";

                    this->b = res.amount * (51 -  this->round_result.size()) * 97 / (100 * (26 - black )); 
                    this->r = res.amount * (51 -  this->round_result.size()) * 97 / (100 * (26 - red ));               
                }
                else
                {
                     this->result = "lose";
        ...
        GSTLIB_SERIALIZE(betitem, (account)(pool)(bet_amount)(bet_nums)(round_result)(pool_record)(color)(round)(result)(b)(r)(c)(blo))
    };
