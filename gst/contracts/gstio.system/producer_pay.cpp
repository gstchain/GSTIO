...
void system_contract::onblock(block_timestamp timestamp, account_name producer)
{
	...
	{ 
		...
		auto time_point = ct - _gstate.last_calculate_time;
		...
		if (time_point > ten_minute) 
	 	{
			...
			double now_base_votes_weight = _gstate.total_producer_vote_weight;
			for(const auto& pd : _producers)
			{
				int64_t producer_per_block_pay = 0;
				if (_gstate.total_unpaid_blocks > 0)
				{
					producer_per_block_pay = (to_per_block_pay * pd.unpaid_blocks) / _gstate.total_unpaid_blocks;
				}
			...
			_gstate.last_calculate_time = current_time();
		}
	}
...
void system_contract::claimrewards(const account_name &owner)
{
	...
	if (usecs_since_last_fill > 0 && _gstate.last_pervote_bucket_fill > 0)
	{
		...
		_gstate.prodtotal_bucket += to_producers;	//2019/03/12	更新可用于投票奖励的总token量
		...
		_gstate.pervote_bucket -= prod.cvunclaim.amount;
		_gstate.perblock_bucket -= prod.cbunclaim.amount;
		_gstate.prodtotal_bucket = _gstate.pervote_bucket + _gstate.perblock_bucket; //更新可用于投票奖励的总token量
		...
		if (producer_per_block_pay > 0)
		{
			...
			(N(gstio.token), {N(gstio.bpay), N(active)},
			 {N(gstio.bpay), owner, prod.cbunclaim, std::string("producer block pay")});
		}
		...
		if (producer_per_vote_pay > 0)
		{
			...
			(N(gstio.token), {N(gstio.vpay), N(active)},
			 {N(gstio.vpay), owner, prod.cvunclaim, std::string("producer vote pay")});
		}
