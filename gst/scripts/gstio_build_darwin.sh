...
while read -r name tester testee brewname uri; do
	...
	(( COUNT++ ))
done < "${REPO_ROOT}/scripts/gstio_build_darwin_deps"
IFS="${var_ifs}"
...
if [ $COUNT -gt 1 ]; then
	...
	case $ANSWER in
		1 | [Yy]* )
			...
			brew tap gstio/gstio # Required to install mongo-cxx-driver with static library
			printf "\\nInstalling Dependencies...\\n"
			# Ignore cmake so we don't install a newer version.
			# Build from source to use local cmake; see homebrew-gstio repo for examples
