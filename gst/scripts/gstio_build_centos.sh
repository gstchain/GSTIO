...
DEP_ARRAY=( 
	...
	ocaml libicu-devel python python-devel python34 \
...
if [ -d /opt/rh/python34 ]; then
	printf "Enabling python34...\\n"
	source /opt/rh/python34/enable || exit 1
	printf " - python34 successfully enabled!\\n"
fi
...
export CPATH="$CPATH:/opt/rh/python34/root/usr/include/python3.3m" # m on the end causes problems with boost finding python3
...
if [ "${BOOSTVERSION}" != "${BOOST_VERSION_MAJOR}0${BOOST_VERSION_MINOR}0${BOOST_VERSION_PATCH}" ]; then
	...
	curl -LO https://sourceforge.net/projects/boost/files/boost/${BOOST_VERSION_MAJOR}.${BOOST_VERSION_MINOR}.${BOOST_VERSION_PATCH}/boost_$BOOST_VERSION.tar.bz2 \
...
function print_instructions() {
	printf "source /opt/rh/python34/enable\\n"
