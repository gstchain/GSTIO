...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->set_code(N(gstio.token), contracts::gstio_token_wasm() );
         chain->set_abi(N(gstio.token), contracts::gstio_token_abi().data() );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   ...
   test.chain->set_code(N(bob), contracts::gstio_token_wasm() );
   test.chain->set_abi(N(bob), contracts::gstio_token_abi().data() );

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), contracts::gstio_token_wasm() );
   test.chain->set_abi(N(charlie), contracts::gstio_token_abi().data() );
...
BOOST_AUTO_TEST_CASE( blacklist_gstio ) { try {
   ...
   tester1.chain->set_code(config::system_account_name, contracts::gstio_token_wasm() );
   tester1.chain->produce_blocks();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   auto auth = authority(gstio::testing::base_tester::get_public_key("alice", "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::gstio_code_name}, 1} );

   tester1.chain->push_action( N(gstio), N(updateauth), N(alice), mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(gstio::testing::base_tester::get_public_key("bob", "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::gstio_code_name}, 1} );
   auth.accounts.push_back( permission_level_weight{{N(bob), config::gstio_code_name}, 1} );

   tester1.chain->push_action( N(gstio), N(updateauth), N(bob), mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(gstio::testing::base_tester::get_public_key("charlie", "active"));
   auth.accounts.push_back( permission_level_weight{{N(charlie), config::gstio_code_name}, 1} );

   tester1.chain->push_action( N(gstio), N(updateauth), N(charlie), mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&]( const transaction_trace_ptr& t) {
      ...
      if( act.account == N(gstio) && act.name == N(onblock) ) return;
