...
BOOST_FIXTURE_TEST_CASE(action_receipt_tests, TESTER) { try {
   ...
   set_code( config::system_account_name, contracts::gstio_bios_wasm() );

   set_code( N(test), contracts::gstio_bios_wasm() );
   set_abi( N(test), contracts::gstio_bios_abi().data() );
	set_code( N(test), contracts::payloadless_wasm() );
...
#warning re-enable deferred transaction replacement test after bug has been fixed
...

   {
      ...
      // But not anymore. Now it should subjectively fail because testapi@additional permission is not unilaterally satisfied by testapi@gstio.code.
      dtt_action dtt_act3;
...
BOOST_FIXTURE_TEST_CASE(multi_index_tests, TESTER) { try {
   ...
   auto check_failure = [this]( action_name a, const char* expected_error_msg ) {
      BOOST_CHECK_EXCEPTION(  push_action( N(testapi), a, N(testapi), {} ),
                              gstio_assert_message_exception,
                              gstio_assert_message_is( expected_error_msg )
      );
   };
