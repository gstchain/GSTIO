...
proxy::proxy( gstio::name self, gstio::name first_receiver, gstio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      gstio::token::transfer_action a( "gstio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, gstio::ignore<std::vector<char>> ) {
