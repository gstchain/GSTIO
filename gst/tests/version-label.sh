#!/bin/bash
# The purpose of this test is to ensure that the output of the "nodgst --version" command matches the version string defined by our CMake files
# If the environment variable BUILDKITE_TAG is empty or unset, this test will echo success
echo '##### Nodgst Version Label Test #####'
...
# orient ourselves
[[ "$GSTIO_ROOT" == '' ]] && GSTIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/gst/')
[[ "$GSTIO_ROOT" == '' ]] && GSTIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/GSTIO/gstio/')
[[ "$GSTIO_ROOT" == '' ]] && GSTIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/build/' | sed 's,/build/,,')
echo "Using GSTIO_ROOT=\"$GSTIO_ROOT\"."
# determine expected value
CMAKE_CACHE="$GSTIO_ROOT/build/CMakeCache.txt"
CMAKE_LISTS="$GSTIO_ROOT/CMakeLists.txt"
if [[ -f "$CMAKE_CACHE" && $(cat "$CMAKE_CACHE" | grep -c 'DOXY_GST_VERSION') > 0 ]]; then
    echo "Parsing \"$CMAKE_CACHE\"..."
    EXPECTED="v$(cat "$CMAKE_CACHE" | grep 'DOXY_GST_VERSION' | cut -d '=' -f 2)"
...
if [[ "$EXPECTED" == '' ]]; then
    ...
    echo ''
    echo '$ cat "$CMAKE_CACHE" | grep "DOXY_GST_VERSION"'
    cat "$CMAKE_CACHE" | grep "DOXY_GST_VERSION"
    echo '$ pwd'
    pwd
    echo '$ ls -la "$GSTIO_ROOT"'
    ls -la "$GSTIO_ROOT"
    echo '$ ls -la "$GSTIO_ROOT/build"'
    ls -la "$GSTIO_ROOT/build"
    exit 1
fi
echo "Expecting \"$EXPECTED\"..."
# get nodgst version
ACTUAL=$($GSTIO_ROOT/build/bin/nodgst --version) || : # nodgst currently returns -1 for --version
# test
