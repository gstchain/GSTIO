...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   // iterate over scope
   gstio::chain_apis::read_only plugin(*(this->control), fc::microseconds::maximum());
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   for (account_name a: accs) {
      push_action( N(gstio.token), N(issue), "gstio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", gstio::chain::asset::from_string("10000.0000 SYS") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "gstio")
         ("maximum_supply", gstio::chain::asset::from_string("1000000000.0000 AAA"));
   push_action(N(gstio.token), N(create), N(gstio.token), act );
   // issue
   for (account_name a: accs) {
      push_action( N(gstio.token), N(issue), "gstio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", gstio::chain::asset::from_string("9999.0000 AAA") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "gstio")
         ("maximum_supply", gstio::chain::asset::from_string("1000000000.0000 CCC"));
   push_action(N(gstio.token), N(create), N(gstio.token), act );
   // issue
   for (account_name a: accs) {
      push_action( N(gstio.token), N(issue), "gstio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", gstio::chain::asset::from_string("7777.0000 CCC") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "gstio")
         ("maximum_supply", gstio::chain::asset::from_string("1000000000.0000 BBB"));
   push_action(N(gstio.token), N(create), N(gstio.token), act );
   // issue
   for (account_name a: accs) {
      push_action( N(gstio.token), N(issue), "gstio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", gstio::chain::asset::from_string("8888.0000 BBB") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   gstio::chain_apis::read_only plugin(*(this->control), fc::microseconds::maximum());
   gstio::chain_apis::read_only::get_table_rows_params p;
   p.code = N(gstio.token);
   p.scope = "inita";
   p.table = N(accounts);
   p.json = true;
   p.index_position = "primary";
   gstio::chain_apis::read_only::get_table_rows_result result = plugin.read_only::get_table_rows(p);
   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("gstio", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("gstio", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("gstio", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("gstio", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( N(gstio), N(bidname), bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
   ...
   bidname(N(inita), N(com), gstio::chain::asset::from_string("10.0000 SYS"));
   bidname(N(initb), N(org), gstio::chain::asset::from_string("11.0000 SYS"));
   bidname(N(initc), N(io), gstio::chain::asset::from_string("12.0000 SYS"));
   bidname(N(initd), N(html), gstio::chain::asset::from_string("14.0000 SYS"));
   produce_blocks(1);
